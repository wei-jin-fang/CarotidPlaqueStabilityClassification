"""
使用GradCAM可视化Mask直接相乘版本的颈动脉斑块分类模型的预测结果
"""
import os
import argparse
import random
...
# 或者更稳妥的写法（推荐）
current_dir = os.path.dirname(os.path.abspath(__file__))  # grandcam/mask_multiply
...
from models.resnet3D_multiply_mask import create_mask_guided_classifier

from gradcam_3d_multiply_mask import GradCAM3DMultiplyMask
...
def visualize_all_patients(args):
    ...
    gradcam = GradCAM3DMultiplyMask(model, target_layer=args.target_layer)
    ...
    for idx, (volume, mask, label) in enumerate(dataloader):
        ...
        # 生成GradCAM (针对预测类别)
        cam = gradcam(volume, mask, target_class=pred_class)
...
def main():
    parser = argparse.ArgumentParser(description='GradCAM可视化 - Multiply-Mask模型')
    ...
    parser.add_argument('--output-dir', type=str,
                       default='./gradcam_visualizations_multiply_mask',
                       help='可视化输出目录')
