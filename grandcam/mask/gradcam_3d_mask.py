"""
3D GradCAM 实现 - 用于带Mask的颈动脉斑块分类模型的可视化
支持双输入模型（图像 + mask）
"""
import torch
...
class GradCAM3DWithMask:
    """
    3D GradCAM for mask-guided 3D medical image classification

    用法:
        model = create_mask_guided_classifier(...)
        gradcam = GradCAM3DWithMask(model, target_layer='layer4')
    ...
    def __init__(self, model, target_layer='layer4'):
        ...
        print(f"✓ GradCAM (Mask版本) 初始化完成，目标层: {target_layer}")
    ...
    def __call__(self, input_tensor, mask_tensor, target_class=None, return_gradients=False):
        ...
        返回:
            如果return_gradients=False: cam [D, H, W]
            如果return_gradients=True: (cam [D, H, W], gradient_map [D, H, W])
        """
        # 确保batch_size=1
        ...
        if return_gradients:
            ...
            # 归一化
            gradient_map = gradient_map.squeeze()  # [D, H, W]
    ...
    def visualize_3d(self, input_tensor, mask_tensor, cam, save_path,
        ...
        plt.suptitle(f'3D GradCAM Visualization (Mask-Guided Model)',
    ...
    def visualize_all_slices(self, input_tensor, mask_tensor, cam, save_dir, patient_id,
        ...
        参数:
            ...
            colormap: OpenCV colormap
            verbose: 是否显示进度
            gradient_map: 梯度空间分布 [D, H, W] (可选，用于调试)
        ...
        for slice_idx in range(D):
            ...
            axes[col_idx].set_title('Mask', fontsize=10)
            axes[col_idx].axis('off')
            col_idx += 1
            ...
            title = f'Slice {slice_idx:03d}/{D-1:03d}'
            if gradient_map is not None:
                title += ' (grad_information included)'
