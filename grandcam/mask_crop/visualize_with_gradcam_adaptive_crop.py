"""
使用GradCAM可视化自适应裁剪版本的Mask引导颈动脉斑块分类模型
输入图像为裁剪后的244x244，所有可视化都基于裁剪后的图像
"""
import os
import argparse
import random
...
# 添加项目根目录到路径
current_dir = os.path.dirname(os.path.abspath(__file__))  # grandcam/mask_crop
...
def visualize_all_patients(args):
    ...
    # 创建目标数据集
    dataset = CarotidPlaqueDatasetWithAdaptiveMask(
        root_dir=args.root_dir,
        mask_dir=args.mask_dir,
        label_excel=args.label_excel,
        transform=transform,
    ...
    # 初始化GradCAM
    print("="*60)
    print(f"初始化GradCAM (目标层: {args.target_layer}, 裁剪后尺寸: 244x244)...")
    print("="*60)

    gradcam = GradCAM3DAdaptiveCrop(model, target_layer=args.target_layer)

    # 对每个患者进行可视化
    print("\n" + "="*60)
    print("开始生成GradCAM可视化（基于裁剪后的244x244图像）...")
    print("="*60 + "\n")

    total_slices = 0

    for idx, (volume, mask, label) in enumerate(dataloader):
        volume = volume.to(device)  # [1, 1, D, 244, 244]
        mask = mask.to(device)      # [1, 1, D, 244, 244]
...
def visualize_specific_patients(args):
    ...
    for count, idx in enumerate(patient_indices):
        ...
        volume = volume.unsqueeze(0).to(device)  # [1, 1, D, 244, 244]
        mask = mask.unsqueeze(0).to(device)      # [1, 1, D, 244, 244]
        label_value = label
...
def main():
    parser = argparse.ArgumentParser(description='GradCAM可视化 - 自适应裁剪版本')
    ...
    parser.add_argument('--output-dir', type=str,
                       default='./gradcam_visualizations_adaptive_crop',
                       help='可视化输出目录')
