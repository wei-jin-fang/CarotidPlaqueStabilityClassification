"""
基于Patch的颈动脉斑块分类训练脚本(融合Excel临床特征) - CLS Token版本

改进:
1. 从ROI区域提取小patch,避免背景干扰
2. 使用CLS Token + Transformer聚合patch特征
3. 融合Excel临床特征(医学指标)
4. 记录patch位置信息,支持后续CLS attention可视化
5. 不使用预训练,从头训练
"""
import os
...
def create_output_folder(base_dir="./output_patch_feature_cls"):
    """创建输出文件夹"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    exp_dir = os.path.join(base_dir, f"train_cls_{timestamp}")
...
def evaluate_test_with_attention(model, test_dataset, device, save_dir):
    """
    详细评估测试集,保存每个样本的预测结果和CLS attention权重
    """
    model.eval()
    results = []
    ...
    with torch.no_grad():
        for idx in range(len(test_dataset)):
            ...
            results.append({
                ...
                'cls_attention_weights': attn_weights_np,
                'positions': positions
            })
    ...
    results_path = os.path.join(save_dir, 'test_predictions_with_cls_attention.pkl')
    with open(results_path, 'wb') as f:
        pickle.dump(results, f)
    print(f"\n详细结果(含CLS attention)已保存: {results_path}")
    ...
    for r in results:
        df_data.append({
            ...
            'top3_cls_attention': str(np.sort(r['cls_attention_weights'])[-3:][::-1])
        })
...
def print_model_summary(model, save_path=None):
    ...
    summary = []
    summary.append("=" * 80)
    summary.append("模型参数统计 (Patch-Feature融合模型 - CLS Token版本)")
    summary.append("=" * 80)
...
def plot_training_curves(history, save_dir):
    ...
    fig.suptitle('Patch-Feature Fusion (CLS Token) Training History', fontsize=16, fontweight='bold')
...
def train(args):
    ...
    print("\n" + "="*60)
    print("加载Patch-Feature融合数据集 (CLS Token版本)...")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("创建Patch-Feature融合分类器 (CLS Token版本)...")
    print("="*60)
    ...
    if len(test_indices) > 0:
        ...
        print("\n" + "="*60)
        print("保存详细预测结果和CLS attention权重...")
        print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description='Patch-Feature融合颈动脉斑块分类训练 (CLS Token版本)')
    ...
    parser.add_argument('--output-dir', type=str, default='./output_patch_feature_cls')
    ...
    parser.add_argument('--vit-depth', type=int, default=1,
                       help='ViT Patch Encoder的Transformer层数')
