...
def create_test_output_folder(base_dir="./test_output"):
    """创建测试输出文件夹"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    test_dir = os.path.join(base_dir, f"test_{timestamp}")
...
def test(args):
    ...
    model = create_classifier(
        num_classes=2,
        pretrained_path=None,  # 不使用预训练，直接加载训练好的模型
...
def main():
    parser = argparse.ArgumentParser(description='颈动脉斑块分类测试')
    ...
    parser.add_argument('--seed', type=int, default=42, help='随机种子（与训练时保持一致）')
