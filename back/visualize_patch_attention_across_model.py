...
def main():
    ...
    parser.add_argument('--max-samples', type=int, default=None,
                       help='最多可视化多少个样本（可选）')
    # 替换原有的 --only-errors 参数，改为更精准的筛选参数
    parser.add_argument('--only-correct-class1', action='store_true',
                    help='只可视化：预测正确 且 真实标签为1 的样本')
    parser.add_argument('--only-correct-class0', action='store_true',
                    help='只可视化：预测正确 且 真实标签为0 的样本')
    ...
    if args.patient_name:
        ...
        # 替换原有的 only_errors 筛选逻辑
    if args.only_correct_class1:
        print(f"\n筛选：预测正确 且 真实标签为1 的样本...")
        correct_class1_indices = []
        for idx in sample_indices:
            try:
                patches, positions, label = dataset[idx]
                # 跳过无有效patch的样本（避免后续报错）
                if len(positions) == 0:
                    print(f"  样本{idx}无有效Patch，跳过筛选")
                    continue
                ...
                # 核心条件：预测正确 + 真实标签为1
                if pred == label_val and label_val == 1:
                    correct_class1_indices.append(idx)
                    print(f"  样本{idx}：真实标签={label_val}, 预测={pred} → 符合条件")
                else:
                    print(f"  样本{idx}：真实标签={label_val}, 预测={pred} → 不符合")
            except Exception as e:
                print(f"  样本{idx}筛选失败：{str(e)}")
                continue

        sample_indices = correct_class1_indices
        print(f"✓ 找到 {len(sample_indices)} 个符合条件的样本（预测正确+真实标签为1）")
    
    if args.only_correct_class0:
        print(f"\n筛选：预测正确 且 真实标签为0 的样本...")
        correct_class0_indices = []
        for idx in sample_indices:
            try:
                patches, positions, label = dataset[idx]
                # 跳过无有效patch的样本（避免后续报错）
                ...
                # 核心条件：预测正确 + 真实标签为0
                if pred == label_val and label_val == 0:
                    correct_class0_indices.append(idx)
                    print(f"  样本{idx}：真实标签={label_val}, 预测={pred} → 符合条件")
                else:
                    print(f"  样本{idx}：真实标签={label_val}, 预测={pred} → 不符合")
            except Exception as e:
        ...
        sample_indices = correct_class0_indices
        print(f"✓ 找到 {len(sample_indices)} 个符合条件的样本（预测正确+真实标签为0）")
