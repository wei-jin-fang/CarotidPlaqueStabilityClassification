...
from models.resnet3D_mask_parse import create_mask_guided_classifier
...
def create_output_folder(base_dir="./output_mask"):
    """创建带时间戳的输出文件夹"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    exp_dir = os.path.join(base_dir, f"train_mask_{timestamp}")
...
def plot_loss_comparison(history, save_dir):
    ...
    fig.suptitle('Loss (Mask-Guided Model)', fontsize=16, fontweight='bold')
...
def train(args):
    ...
    # 创建数据集副本
    train_dataset = CarotidPlaqueDatasetWithMask(
        root_dir=args.root_dir,
        mask_dir=args.mask_dir,
        label_excel=args.label_excel,
    ...
    val_dataset = CarotidPlaqueDatasetWithMask(
        root_dir=args.root_dir,
        mask_dir=args.mask_dir,
        label_excel=args.label_excel,
        transform=val_transform,
    ...
    for epoch in range(args.epochs):
        ...
        # 保存最佳模型
        if epoch in [15] or test_metrics_temp['accuracy'] >= 0.71:
...
def main():
    parser = argparse.ArgumentParser(description='颈动脉斑块分类训练 - Mask引导')
    ...
    parser.add_argument('--label-excel', type=str,
                       ...
                       help='标签Excel文件')
    parser.add_argument('--output-dir', type=str, default='./output_mask',
                       help='输出目录')
    ...
    # 原有参数（如 num_classes 必须的）
    parser.add_argument('--num-classes', type=int, default=2, help='分类类别数')
    # 新增的可调分类头参数
    parser.add_argument('--fc1_out', type=int, default=256, help='Output dim of fc1 (default: 256)')
    parser.add_argument('--fc2_out', type=int, default=128, help='Output dim of fc2 (default: 128)')
    parser.add_argument('--dropout1', type=float, default=0.5, help='Dropout rate after fc1 (default: 0.5)')
    parser.add_argument('--dropout2', type=float, default=0.5, help='Dropout rate after fc2 (default: 0.5)')
