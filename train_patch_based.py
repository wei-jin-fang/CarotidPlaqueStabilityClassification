...
def create_output_folder(base_dir="./output_patch_based"):
    ...
    exp_dir = os.path.join(base_dir, f"train_patch_{timestamp}")
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, (patches_list, positions_list, labels) in enumerate(dataloader):
        # Padding patches
...
def print_model_summary(model, save_path=None):
    ...
    for name, info in params_info['by_module'].items():
        summary.append(f"  {name:20s}: {info['total']:>10,} 总参数, {info['trainable']:>10,} 可训练")
    summary.append("=" * 80)
...
def plot_training_curves(history, save_dir):
    ...
    # 创建1行2列（横向）或2行1列（纵向）子图，这里用2行1列更适合Loss对比
    fig, axes = plt.subplots(2, 1, figsize=(10, 8))
    fig.suptitle('Patch-based Training Loss History', fontsize=16, fontweight='bold')
    ...
    axes[0].set_xlim(1, len(epochs))  # x轴范围固定，更美观
    ...
    axes[1].set_xlim(1, len(epochs))  # x轴范围和训练Loss保持一致
...
def train(args):
    ...
    # 创建子数据集
    train_dataset = PatchBasedCarotidDataset(
        root_dir=args.root_dir,
        mask_dir=args.mask_dir,
        label_excel=args.label_excel,
        patch_size=args.patch_size,
    ...
    val_dataset = PatchBasedCarotidDataset(
        root_dir=args.root_dir,
        mask_dir=args.mask_dir,
        label_excel=args.label_excel,
        patch_size=args.patch_size,
...
def main():
    parser = argparse.ArgumentParser(description='Patch-based颈动脉斑块分类训练')
