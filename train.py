...
def create_output_folder(base_dir="./output"):
    ...
    exp_dir = os.path.join(base_dir, f"train_{timestamp}")
...
def calculate_class_weights(dataset, device, pos_scale=1.2, neg_scale=1.0):
    """
    计算类别权重并允许手动放大/缩小某个类别的权重
    
    Args:
        dataset: 你的数据集对象
        device: torch.device
        pos_scale: 正类（label=1）的额外放大系数，默认1.0
        neg_scale: 负类（label=0）的额外放大系数，默认1.0
    
    Returns:
        torch.Tensor: [weight_for_class0, weight_for_class1]
    """
    class_counts = [0, 0]
...
def validate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for volumes, labels in dataloader:
            ...
            all_preds.extend(preds.cpu().numpy())
            all_labels.extend(labels.cpu().numpy())
            all_probs.extend(probs[:, 1].cpu().numpy())  # 类别1的概率
...
def evaluate_test_set_detailed(model, test_dataset, test_indices, device, save_dir):
    """
    详细评估测试集，记录每个样本的预测结果
    ...
    参数:
        model: 训练好的模型
        test_dataset: 测试数据集
        test_indices: 测试集索引（用于获取患者名称）
        device: 设备
        save_dir: 保存目录

    返回:
        DataFrame: 包含每个样本详细信息的DataFrame
    """
    model.eval()
...
def plot_loss_comparison(history, save_dir):
    ...
    参数:
        history: 包含所有epoch的训练历史（包括epoch 0）
        save_dir: 保存目录
    """
    # 提取数据
    epochs_with_0 = list(range(len(history['train_loss'])))  # [0, 1, 2, ..., n]
    epochs_from_1 = list(range(1, len(history['train_loss'])))  # [1, 2, ..., n]
    ...
    fig.suptitle('Loss', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('training Loss (1)', fontsize=13, fontweight='bold')
    ...
    ax2.set_title('val Loss (1)', fontsize=13, fontweight='bold')
    ...
    ax3.scatter(0, train_loss[0], color='green', s=150, zorder=5,
               marker='*', label='init Loss (Epoch 0)')
    ax3.set_xlabel('Epoch', fontsize=12)
    ax3.set_ylabel('Loss', fontsize=12)
    ax3.set_title('traing Loss (0)', fontsize=13, fontweight='bold')
    ...
    ax4.set_title('val loss (0)', fontsize=13, fontweight='bold')
...
def train(args):
    ...
    test_dataset = CarotidPlaqueDataset(
        root_dir=args.root_dir,
        label_excel=args.label_excel,
        transform=val_transform,
        keep_middle_n=args.depth,
    ...
    best_epoch = 0
    history = {
        'train_loss': [initial_train_loss],  # 包含第0轮
        'train_acc': [np.nan],               # 第0轮用NaN占位（没有训练）
        'val_loss': [initial_val_loss],      # 包含第0轮
        'val_acc': [np.nan],                 # 第0轮用NaN占位（没有验证指标）
        'val_f1': [np.nan],                  # 第0轮用NaN占位
        'val_auc': [np.nan]                  # 第0轮用NaN占位
    }
    ...
    if len(test_indices) > 0:
        ...
        test_predictions_df = evaluate_test_set_detailed(
            model, test_dataset, test_indices, device, sub_dirs['results']
        )
...
def main():
    parser = argparse.ArgumentParser(description='颈动脉斑块分类训练')
